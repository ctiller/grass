param(
    ...
    [string]$OutputDirectory = ".lake/disasm/c-sequence"
)
...
$records = @()
foreach ($case in @("sequence_safe", "sequence_oob")) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Independent EXE disassembly failed: $case" }
    ...
    $exports = & $dumper /nologo /exports $library
    if ($LASTEXITCODE -ne 0) { throw "Independent DLL export listing failed: $case" }
    $exports | Set-Content -LiteralPath (Join-Path $output "$case.dll.exports.txt")
    $records += [ordered]@{
        case = $case
...
[ordered]@{
    schema = "grass.disasm.c-sequence-build.v1"
    revision = $revision
    ...
    scope = "two volatile compiler store attempts under a declared RCX object contract"
    assurance = "compiler artifacts and dumpbin disassembly only; no execution verdict"
    fixtures = $records
} | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $output "manifest.json")
Write-Output "Built sequence C fixtures in $output. No fixture binary was executed."
