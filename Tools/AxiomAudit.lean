...
The audit therefore walks `Grass/` on disk and passes every discovered module
directly to `Lean.importModules`. It then checks the resulting environment's
module header before scanning declarations. There is no hand-maintained module
registry to drift or acquire merge markers.

The dynamically assembled import set means this tool loads every leaf, which
...
"}"
  ...
  let mut directAxiomFindings : Array Name := #[]
  let mut findings : Array (Name × Name) := #[]
  for (name, info) in env.constants.toList do
    unless isAudited name do continue
    audited := audited + 1
    -- `collectAxioms` reports what a declaration depends on, but an axiom
    -- declaration is itself a trust root and is not necessarily returned as
    -- its own dependency. Reject it directly before following dependencies.
    if info matches .axiomInfo _ then
      unless allowedAxioms.contains name do
        directAxiomFindings := directAxiomFindings.push name
...
"}"
  if directAxiomFindings.isEmpty && findings.isEmpty then
    ...
    let directLines := directAxiomFindings.map fun name =>
      m!"  {name} is a directly declared axiom"
...
{allowedAxioms}\n{MessageData.joinSep (directLines ++ lines).toList "\n"}"
