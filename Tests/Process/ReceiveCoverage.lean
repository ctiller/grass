...
theorem cursor_change_has_delivery
    ...
    ∃ (occurrence : EdgeOccurrence plan.topology plan.message target)
      (delivery : plan.Delivers before after target key occurrence),
      transition = .receive target key occurrence delivery := by
  classical
  cases transition with
  | receive edge session occurrence delivery =>
    by_cases hit : NetworkFragment.session target key = NetworkFragment.session edge session
    · cases hit
      exact ⟨occurrence, delivery, rfl⟩
    · exact False.elim (changed (cursor_frame
        (delivery.scope (.session target key) (by simp [hit]))))
