...
example (value : Byte) :
    normalize takeByte writeByte (writeByte value) = some (writeByte value) :=
  normalize_write takeByte_realizes writeByte_realizes value

example (value : Byte) :
    normalize takeByte writeByte (writeByte value ++ Vec.singleton 0xaa) = none := by
  unfold normalize
  rw [takeByte_writeByte_append]
  exact if_neg (by decide)

example (input output : Std.Logical.ByteArray)
    (normalized : normalize takeByte writeByte input = some output) :
    normalize takeByte writeByte output = some output :=
  normalize_idempotent takeByte_realizes writeByte_realizes normalized
