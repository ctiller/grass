...
def structurallySelectedOnly : LoopSelection Nat Terminal where
  graph := graph
  selected := [
    { headerObligation with invariant := fun _ => False },
    { doneObligation with progress := .frontier fun _ _ => False }
  ]

-- `LoopSelection.wellFormed` records obligations but does not discharge them.
example : structurallySelectedOnly.WellFormed := by decide
