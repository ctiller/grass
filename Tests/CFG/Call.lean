...
example : provider.OutcomesWellFormed :=
  CallContract.outcomesWellFormed_of_wellFormed provider (by decide)
example : provider.entryStack.WellFormed :=
  CallContract.entryStackWellFormed_of_wellFormed provider (by decide)
...
example : good.actualEntryStack = good.contract.entryStack :=
...
example : localGood.WellFormed := by decide
...
def malformedOutcomeStack : CallContract Nat where
  requires := fun _ => True
  entryStack := callStack
  outcomes := [{ normal with stack := ⟨0, [⟨⟨"test.call", "dup"⟩⟩,
    ⟨⟨"test.call", "dup"⟩⟩]⟩ }]

example : ¬ malformedOutcomeStack.WellFormed := by decide
