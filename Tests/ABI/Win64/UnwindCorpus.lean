...
should be. The `win64-unwind-differential` binary in `tools/grass-tools`
assembles the prologue and compares. That catches more than the byte layout,
because the model's `PlacedOp.codeOffset` values are computed here from
instruction *lengths*:
