...
example : (backend.leave frame).expand =
    [.add frame.plan.subtracted, .pop .r12, .pop .rbx] := by native_decide
example : (backend.spill? frame ⟨"slot"⟩ .rax).map Source.expand =
    some [.spill .rax 32] := by native_decide
example : (backend.reload? frame ⟨"slot"⟩ .rax).map Source.expand =
    some [.reload .rax 32] := by native_decide
example : backend.reload? frame ⟨"missing"⟩ .rax = none := by native_decide
