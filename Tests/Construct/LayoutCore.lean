...
example : (good.lookup? ⟨"value"⟩).isSome = true ↔
    (⟨"value"⟩ : Grass.Core.Name) ∈ good.fieldNames :=
  good.lookup?_isSome_iff_mem_fieldNames ⟨"value"⟩
example : ∃ placed, good.lookup? ⟨"value"⟩ = some placed :=
  good.fieldForName ⟨"value"⟩ (by decide)
example : good.fieldNames.Nodup := good.fieldNamesNodup_of_wellFormed (by decide)
example : good.FieldsWellFormed := good.fieldsWellFormed_of_wellFormed (by decide)
example : good.FieldsDisjoint := good.fieldsDisjoint_of_wellFormed (by decide)
