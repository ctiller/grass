...
This is the reader. `tools/grass-tools`'s `source-liveness` fetches each `url`
and checks whether `livenessProbe` appears in what comes back, then compares
that against the recorded `retrieval`. A recorded `.verified` whose probe string
is absent is a finding; so is a recorded `.dead` whose probe string is present,
because a document that came back is no longer a release blocker.
