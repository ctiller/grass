...
by a different route. `tools/grass-tools`'s `axiom-audit` had already learned
this lesson about import lists and walks the disk instead; this file does the
same for citations.
...
than a disk walk, which is the hazard `tools/grass-tools`'s `axiom-audit` solves
properly; closing that here is an open obligation.
...
declaration-name walk -- then a separate meta-program, now generated by
`tools/grass-tools`'s `docstring-audit` -- both named the new module, their disk
walks being real, but the ledger did not, so after adding the two imports those
guards demanded, the ledger's summary came back byte-identical to baseline.
...
The same walk `tools/grass-tools`'s `axiom-audit` performs, and for the same
reason: a hand-written module list is a coverage claim nobody checks.
-/
...
`tools/grass-tools`'s `axiom-audit` had the same defect and carries the same fix
in the Lean it generates.
-/
...
run_cmd liftTermElabM do
  let env ← getEnv
  -- Coverage first, for the reason axiom_audit.rs in tools/grass-tools gives:
  -- a gate over half the tree is not a gate. An audited module that is not
  -- imported here contributes no declarations and silently narrows the
  -- obligation.
