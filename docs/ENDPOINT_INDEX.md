...
|---|---|---|
| Hello entry prefix | The program-specific execution template and prefix-slicing gate are retired under the specialization removal plan. | General full-source verification and emission remain incomplete. |
| Incoming WriteFile arguments | `Grass/Refinement/Console/WriteFileSourceEntry.lean` checks supplied buffer/count/fifth arguments at the actual CALL through the existing preparation and stack-plan producers. The program-specific static/count composition has been removed. | The [incoming-state contract](PLATFORM_ABI.md#incoming-state-contract) requires exact provenance, range, memory and CALL binding. Generic source-to-argument VC generation remains incomplete. |
