...
The expected source makes that adjacency a value rather than asking
`verify_assembly` to rediscover it. `Bindings.lean` constructs
`sortImplementationBinding : ImplementationBinding sortExpandedSource
sortRequirement` from the exact source scope, descriptor representation,
banked model certificate, assembly-to-model theorem, and
component-to-program theorem. A merge-body edit first invalidates
