...
The `spike-sources` tool currently proves only the normalized authored-source
snapshot equality; it must never be cited as evidence for expansion,
certificate, or artifact reproducibility. It accepts spike numbers as positional
arguments for a focused check, and refuses a selection that names no spike
rather than reporting a clean corpus over nothing. Once implementation begins, CI also runs clean-output generation
