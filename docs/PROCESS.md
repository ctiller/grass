...
the complete interface. In particular the current `ProcessVocabulary`,
`DriverBoundary`, `ProcessPlan`, and sequential adapter do not yet contain the
authority-indexed provider-demand envelope, finite selected-protocol image,
...
structure ProcessVocabulary where
  ...
  demandOrigins : (demand : Demand) ->
    RegisteredOperationOrigins providerDemands demand
...
The vocabulary also selects the finite typed provider-demand envelope and
connects every demand constructor to its exact, possibly empty or multi-origin,
provider-demand subfamily in that envelope.
...
structure ProcessPlan (registry : ProtocolRegistry) (boundary : DriverBoundary)
  ...
  selectedProtocols : SelectedProtocolImage registry toProcessTopologyCore
  ...
  providerCoverage : EveryBoundaryAndSelectedRoleDemandOriginOccursIn
    boundary registry toProcessTopologyCore selectedProtocols providerDemands
...
inductive ProcessPlanSource {R : Type u} [ResourceModel R]
    ...
    (boundary : DriverBoundary)
  | sequential
      (program : CertifiedDirectProgram boundary)
...
`plan.providerDemands`, the conservative union over the boundary and the
plan's finite exact `selectedProtocols` image; in the blended case they are the exact closed union recorded
...
structure SequentialPendingSemantics
  ...
  WaitingOccurrence : Type
  point : WaitingOccurrence -> machine.State
  demand : (waiting : WaitingOccurrence) -> EffectDemand boundary
  resume : (waiting : WaitingOccurrence) ->
    EffectResult (demand waiting) -> machine.State
  isWaiting : forall waiting,
    machine.decide (point waiting) = .effect (demand waiting) (resume waiting)
  History : WaitingOccurrence -> Type
  root : (waiting : WaitingOccurrence) -> History waiting
  advance : PendingHistoryExtensionRelation machine WaitingOccurrence History
  advanceProper : EveryPendingAdvanceIsProper advance
  observationsExact : EveryPendingAdvanceEmitsExactNewPrefixObservations advance
  preserves : EveryPendingAdvancePreservesMachineInvariant machine advance

def SequentialPendingSemantics.atomic
    (atomic : EveryEffectTheoryHasNoProperPendingExtensionOrObservation machine) :
...
`SequentialAdapter.elaborateMachine` translates this syntax plus its pending
...
`DirectProgramRealizes spec (certifiedMachine machine)`.

The ordinary atomic API infers `SequentialPendingSemantics.atomic`; authors do
not add a field. An Effect-generated or custom streaming adapter instead stores
the exact pending history in adapter-owned state and uses `advance` without
resuming the source continuation or consuming its occurrence.
...
structure CertifiedDirectProgram (boundary : DriverBoundary) where
  program : DirectRelationalProgram boundary
  originDemands : ProviderDemandFamily := boundary.providerDemands
  originsExact : originDemands.ExtEq boundary.providerDemands
  operationOrigins : forall occurrence,
    occurrence \u2208 DynamicOccurrences program.Initial program.Step ->
    RegisteredOperationOrigins originDemands occurrence
  derivation : DirectProgramDerivation program
```
...
`CertifiedDirectProgram.operationOrigins` identifies the exact finite provider
demand subfamily attached to each abstract operation and proves all of its
origins occur in the boundary's family. The subfamily may be empty and may
...
structure StandardSequentialRealization {R : Type u} [ResourceModel R]
    {resources : R} (spec : SpecProcess resources) where
  program : CertifiedDirectProgram spec.driverBoundary
  correct : DirectProgramRealizes spec program
...
def ProcessRealization.standard
  ...
  ProcessRealization.sequential
    ...
    (selected.realization.transport selected.exactSpec).derivation
...
def ProcessRealization.serialize
    ...
    CertifiedDirectProgram r.boundary
