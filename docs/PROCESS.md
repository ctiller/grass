...
structure ProcessVocabulary where
  ...
  demandOrigin : (demand : Demand) ->
    RegisteredOperationOrigin providerDemands demand
...
connects every demand constructor to a registered origin in that envelope.
...
inductive ProcessPlanSource {R : Type u} [ResourceModel R]
  ...
  | sequential
      ...
      (correct : DirectProgramRealizes spec program derivation)
  | explicit
      (registry : ProtocolRegistry)
      (plan : ProcessPlan registry boundary)
...
`program.originDemands`; in the explicit case they are exactly
`plan.providerDemands`, the conservative union over the boundary and complete
protocol registry; in the blended case they are the exact closed union recorded
...
retains the exact `DirectProgramDerivation`; the standard Effect adapter's
...
plan; its separate local origin proof connects every generated site to the
...
`operationOrigin` identifies the registered constructor which created each
abstract operation and proves its origin occurs in the boundary's exact demand
family. `originDemands` is the displayed definition, never an author field plus
an equality witness. The finite site inventory establishes local occurrence
provenance; it does not duplicate stable provider obligations once per call site.
...
`DirectProgramRealizes spec program derivation` is indexed by the exact
derivation. The ordinary direct constructor carries the authored relational
...
theorem flatten_sequential_roundtrip
    ...
    (correct : DirectProgramRealizes spec program derivation) :
  Bisimilar
    (ProcessRealization.sequential program derivation correct).flatten
    program
```
...
theorem serialize_as_child
    (r : ProcessRealization spec) (serial : SerializablePlan r) :
    ProcessRealization.sequential
      (r.serialize serial) (serializeDerivation r serial)
      (serialize_correct r serial)
      ≈ r
```
