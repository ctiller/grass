...
structure DemandProviderEnvelope (Demand : Type) where
  demands : ProviderDemandFamily
  origins : Demand -> RegisteredOperationOrigins demands

structure CertifiedProcessVocabulary (vocabulary : ProcessVocabulary) where
  providers : DemandProviderEnvelope vocabulary.Demand
...
structure CertifiedDriverBoundary (boundary : DriverBoundary) where
  providers : DemandProviderEnvelope boundary.Demand
...
structure PendingInteractionModel (boundary : DriverBoundary) where
  ...
  proper : Extends first second -> first != second
...
structure SequentialPendingSemantics
  ...
  advanceExact : advance before after <->
    ...
    model.Extends before.2 (sameDemandTransport after.2)
...
constructor. `SequentialMachineRealizes` is indexed by the same exact model and
connects it to the specification. An Effect-generated adapter separately proves
