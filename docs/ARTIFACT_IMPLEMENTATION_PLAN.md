...
| axiom-audit coverage import | shared integration | g-build, under `c-x86:13`/`:16` route | resolved (`g-build:21`) | `Tools/AxiomAudit.lean` imports every grammar module; the transitive audit passes |
| declaration-list coverage import | shared integration | g-build, under `c-x86:13`/`:16` route | resolved (`g-build:19`) | `Tools/DeclNames.lean` imports every grammar module; `DocstringAudit.py` passes |
