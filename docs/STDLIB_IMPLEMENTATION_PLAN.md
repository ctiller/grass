...
2. `./tools/grass-tools/target/release/axiom-audit` reports no axiom outside the
   ...
   time, and each version stopped tracking the library as it grew. It can no
   longer stop: the `axiom-audit` binary derives its coverage set by walking the
   source tree and checks that walk against the environment's own
   `moduleNames`, so there is no hand-written import list left to fall behind —
   which is the exact failure this criterion was restated twice for. It is also
   not evidence about the `@[extern]` boundary of §3.9, because an `@[extern]`
   is not an axiom.
