...
Every one of these is a binary in `tools/grass-tools` rather than a
`lake env lean` invocation or a PowerShell script, so this list runs
unchanged on Linux as well as Windows. CI builds and runs the same
binaries, so a local run and the gate are the same program.
