...
| AUD-004: independent x86 encoding/boundary validation | Revisited at generic receipt `b1a9639c`; spikes deferred the campaign to preserve required instruction-adapter capacity | Revisit at the first actual fetched x86 stack-instruction execution receipt delivery (PUSH/SUB). The generic Op receipt adds no x86 encoding behavior. x86 owns the campaign; retained replay and mutation evidence remain required for closure |
