...
### 10.120 The interruption reason was flat, and the ending could not say what it abandoned

`agent-bus` ruling `g-design:67` on `c-process:66`, implemented. `docs/PROCESS.md`
§2 has said `InterruptReason : Demand -> Type` since before this branch; the Lean
had it as a single `Type u` in forty places across twenty files. g-design took
split option (c): index it now, defer the facet-carrying topology to §13.

The mechanical part is what it looks like — `fun _ => PEmpty` in eighteen fixture
vocabularies. Three consequences were not mechanical.

**One. `ProcessLifecycle.interrupted` had to carry the demand**, because the
reason's type names it, and that made a law statable that had not been.
`EndsInstance.endingIsEarned` said `outstanding ≠ 0` — a process holding
*something* may abandon *something* — and under a flat reason that was the
strongest thing it could say, because the ending carried no demand to compare
against the bag. It now says `demand ∈ outstanding`.
`Tests/Process/EndingFixtures.lean`'s `no_abandoning_a_demand_it_never_held` is
the case the old field admitted: an instance holding a `tick`, recorded as having
abandoned a `log` it never issued. That is the fifth time in this milestone a
field's docstring named a property its type did not carry, and the first time the
gap was closed by a type change made for another reason entirely.

**Two. `VocabularyDelivery` had to split**, and the argument for splitting is the
module's own. It bundled the three exceptional classes on the stated ground that
none of them carries a dependent result — "faults carry no dependent result" —
which is why `carryFault` mentions no demand, and why
`Tests/Process/DeliveryFixtures.lean`'s `carries_fault_without_demand` can deliver
a fault into a target whose `Demand` is `PEmpty`. Indexing falsifies that premise
for exactly one of the three. Classifying an interruption reason now requires
saying which of the target's demands the abandoned one becomes, and a
`demand : source.Demand → target.Demand` field would make the whole record
unconstructible whenever the target names no demands — taking the fault
classification down with it and deleting the fixture the module had already been
rewritten once to satisfy. So the record splits along the line the indexing drew:
`VocabularyDelivery` keeps `fault` and `violation`, and a new `InterruptDelivery`
carries the demand translation and the indexed reason classifier.

Decision 121's totality is not weakened by the split, and the emptiness lemma got
stronger. `InterruptDelivery.source_empty_of_target_empty` needs the target's
class empty *at one demand* rather than everywhere, so a target that can be
interrupted while writing and not while computing proves the computing case
unreachable without claiming anything about the write.
`Tests/Process/DeliveryFixtures.lean`'s `mixed` is that vocabulary, and
`no_interrupt_delivery_worker_to_ledger` is the other new fact: one pair of
vocabularies admits a fault delivery and no interruption delivery at all, which
is a distinction the bundled record could not draw in either direction.

**Three. `docs/PROCESS.md`'s own lifecycle families were stale against its own
§2.** `ProcessLifecycle.interrupted` and `ChildLifecycleEvent.interrupted` both
took a bare `InterruptReason` while the vocabulary block four hundred lines above
declared it demand-indexed. That is on main, not on this branch, and it is the
ordinary consequence of one normative document being edited in two places; it is
fixed here because the merge is where it became visible.

### 10.121 What the ruling deferred, and what that costs until it lands

Ruling item 3 of `g-design:67` keeps decision 122's facet-carrying
`ProcessTopology` as normative text and defers its *implementation* to an
explicit later milestone. §13 is the record it asked for. The entry is here so a
reader of §10 does not conclude from the merge that the interface was settled: it
was **staged**, and the implemented `ProcessPlan` is provisional until §13.1's
gate is met — it may not claim to discharge cancellation or supervision
...
**Numbering note.** §10.125 to §10.128 are the four ruling implementations on
`refs/heads/agent/c-process/m4-rulings`, nominated as `c-process:74` and not yet
merged. This entry takes the next free number rather than one of theirs; a gap in
this file is cheaper than two branches claiming §10.125.
...
exit criteria still need. Written because §10 has grown past a hundred entries
and a reader needs to know which of them are open.

**This section is generated by reading, and has twice been wrong about what it
summarises** — see the correction under the witness table, and §10.105. Treat its
tables as a reader's index and the files themselves as the record.
...
| `LogicalProcessNetworkCore.WellFormed`, `ProcessPlan.Sound` | nothing; `terminated_result_is_exact` took a `Sound` nothing had supplied | `FrontierFixtures.lean` and `WorldFixtures.lean` vacuously, then `PreservationFixtures.lean`'s `spawned_is_wellFormed`, which is not |
| `ViewFacet`, and `viewAccepts` non-vacuously | §10.56 and §10.99: no specification had a view, and the acceptance clause could not see the state | `Tests/Process/ViewFixtures.lean` |
| `NetworkStep` at an allocating transition | every existing step was non-allocating, at an instance-free world | `Tests/Process/PreservationFixtures.lean` |
...
## 13. Deferred with an owner and a gate

One item, recorded because `agent-bus` ruling `g-design:67` requires a deferral to
name an owner and an acceptance gate rather than to disappear into a merge.

### 13.1 The facet-carrying `ProcessTopology`

**Owner.** `c-process`.

**What.** `docs/PROCESS.md`'s
...
with `ProcessTopology.allCancellationContracts` and
`ProcessTopology.allSupervisionContracts`, and `ProcessPlan` extending it.
`docs/DECISIONS.md` decision 122 is the ratification and §10.8 is where this plan
first recorded the shape.

**Current state.** No such structure exists in `Grass/`. `ProcessPlan` carries
`topology : ProcessTopologyCore` as a field — a shape §10.52 and decision 128
already moved once — and there are no facet fields anywhere in the corpus.

**Why it was deferred rather than done.** It is a new structure at the
topology/plan boundary, and the M4 candidate is already large enough that
`e-reviewer` is reviewing it a second time. g-design's ruling calls this staging,
not supersession of decision 122.

**What it costs until it lands**, and this is the part that is not free. The
implemented `ProcessPlan` is *provisional*:

* it may not claim to discharge cancellation or supervision requirements, and no
  document or docstring may say that it does;
* it may not be consumed as a complete `ProcessPlan` by `VerifiedProgram`;
* `docs/PROCESS.md` carries an implementation-status note beside the target text
  so a reader does not mistake it for current code.

**Acceptance gate.** The milestone is met when all five hold.

1. `ProcessTopology` exists in `Grass/` with `facets` and `facetsExact`, and
   `requiredTopologyFacets` derives the demanded family from
   `boundary.requirements` rather than from an author-supplied second demand set.
   §10.64's complaint about author-chosen predicates is why this is spelled out.
2. The two aggregate theorems are proved, not stated.
3. `ProcessPlan` extends it, and `Grass/Process/Network/Plan.lean`'s `topology`
   field is gone rather than shadowed.
4. There is a plan-level witness with a **non-empty** facet family. An empty
   family is supplied automatically under decision 122, so a fixture that selects
   one proves nothing — this milestone has spent eight review rounds on records
   nothing inhabits and on laws that cannot fail.
5. A falsifying fixture: a topology whose facets do not meet the demanded
   contracts, refused by `facetsExact`.

**Not owed by this gate.** A boundary whose `requirements` are empty must still
author no facet field — decision 122's ceremony clause. That is a property of the
construction and is already checked by the quiescent fixtures.
