...
use clap::Parser;
use cli::{
    Command, DependencyCmd, HandoffCmd, IssueCmd, LifecycleCmd, PlanCmd, ReviewCmd, ScopeCmd,
};
...
fn run(cli: cli::Cli) -> error::AbResult<()> {
    if let Command::BootstrapInit(args) = &cli.command {
        ...
        println!(
            "bootstrapped agent-bus with coordinators: {:?}",
            args.coordinator
        );
        return Ok(());
    }
    ...
    match &cli.command {
        ...
        Command::Issue(cmd) => match cmd {
            ...
            IssueCmd::Acknowledge {
                agent,
                issue_id,
                note,
            } => commands::issue_acknowledge(&ctx, agent, issue_id, note),
            IssueCmd::Resolve {
                agent,
                issue_id,
                file,
            } => commands::issue_resolve(&ctx, agent, issue_id, file),
            IssueCmd::Reject {
                agent,
                issue_id,
                file,
            } => commands::issue_reject(&ctx, agent, issue_id, file),
            IssueCmd::Reassign {
                agent,
                issue_id,
                new_target,
        ...
        Command::Handoff(cmd) => match cmd {
            ...
            HandoffCmd::Accept {
                ...
                handoff_id,
                note,
            } => commands::handoff_accept(&ctx, agent, handoff_id, note),
            HandoffCmd::Decline {
                agent,
                handoff_id,
                reason,
        ...
        Command::Review(cmd) => match cmd {
            ...
            ReviewCmd::Take {
                ...
                nomination,
                note,
            } => review_cmds::take(&ctx, agent, nomination, note),
            ReviewCmd::Decline {
                agent,
                nomination,
                reason,
            } => review_cmds::decline(&ctx, agent, nomination, reason),
            ...
            ReviewCmd::Withdraw {
                ...
                nomination,
                reason,
            } => review_cmds::withdraw(&ctx, agent, nomination, reason),
        ...
        Command::PrepareMerge(a) => {
            review_cmds::prepare_merge(&ctx, &a.agent, &a.nomination, &a.reviewed_commit)
        }
        Command::MergeReady(a) => {
            review_cmds::merge_ready(&ctx, &a.agent, &a.authorization, a.json)
        }
        ...
        Command::Validate(a) => validate_cmd::validate(
            &ctx,
            a.incremental.as_deref(),
            a.linked,
            a.quarantine_invalid,
            a.json,
        ),
