...
//!
//! `docs/REVIEW.md` section "Documentation assurance and mechanized gates" owns
//! the corpus-wide vocabulary and evidence rule. This audit implements the
//! deliberately under-reporting, machine-recognizable subset ratified there for
//! `Grass/**/*.lean`. `docs/MEMORY_IMPLEMENTATION_PLAN.md` section 3.10 cites
//! that corpus rule rather than defining a competing list.
//!
...
///
/// Comparison is case-sensitive, which is what `pathlib` does on the Linux
/// runner that gates this branch and therefore what this tool reproduces.
///
/// Run on Windows the Python does *not* agree, and the reason is worth stating
