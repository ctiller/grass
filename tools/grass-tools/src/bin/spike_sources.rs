...
mod tests {
    ...
    fn content_comparison_is_ordinal() {
        ...
        // And the ordinary half of "ordinal", which the zero-width joiner above
        // does not reach: Lean identifiers are case-sensitive, so `Nat` and `nat`
        // are different declarations and a document showing one while the file
        // holds the other is drift. A comparison that ignored ASCII case would
        // still fail the case above and pass this one.
        let mut cased = BTreeMap::new();
        cased.insert("Main.lean".to_string(), "def main : Nat := 1".to_string());
        let findings = compare_manifest(
            1,
            "Spikes/1_Hello_World",
            "docs/SPIKE_1.md",
            &["Main.lean".to_string()],
            &cased,
            &mut |_| Ok("def Main : nat := 1".to_string()),
        )
        .expect("compared");
        assert_eq!(
            findings,
