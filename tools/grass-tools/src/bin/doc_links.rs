...
mod tests {
    ...
    fn percent_escapes_are_decoded_and_malformed_ones_are_left_alone() {
        ...
        // Through `target_path`, not just as its own function. Testing the decoder
        // in isolation says nothing about whether anything calls it: drop the call
        // and every assertion above still passes, while `[a](a%20b.md)` starts
        // being resolved against a file named `a%20b.md` that nobody wrote.
        let scheme = Regex::new(SCHEME_PATTERN).expect("pattern");
        assert_eq!(
            target_path(&scheme, "docs/a%20b.md#top"),
            Some("docs/a b.md".to_string())
        );
    }

    #[test]
    fn a_percent_escaped_link_resolves_to_the_file_it_names() {
        // The end of that same thread: a real document, a real file with a space
        // in its name, and the link written the way a Markdown editor escapes it.
        let dir = tempfile::tempdir().expect("tempdir");
        let root = dir.path();
        fs::create_dir_all(root.join("docs")).expect("mkdir");
        fs::write(root.join("docs/a b.md"), "").expect("write");
        fs::write(root.join("README.md"), "[spaced](docs/a%20b.md)\n").expect("write");

        let scanned = scan(root).expect("scan");
        assert_eq!(scanned.examined, 1);
        assert!(
            scanned.failures.is_empty(),
            "an escaped link must resolve to the file it names: {:?}",
            scanned.failures
        );
    }
