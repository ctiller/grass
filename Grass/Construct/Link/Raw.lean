...
Relocation meaning and import identity are parameters supplied by ISA/ABI and
platform facets. `CertifiedRelocatableFragment` separately pairs a structurally
checked value with the exact source-to-payload relation supplied by that backend.
-/

namespace Grass.Construct.Link

open Grass Grass.CFG Grass.Construct.Fragment Grass.Std.Logical
...
/-- Exact authored occurrence range within initialized section bytes. -/
...
structure RelocatableFragment (RelocKind : Type u) (ImportIdentity : Type v) where
  id : LinkFragmentId
...
def hasSymbol (fragment : RelocatableFragment RelocKind ImportIdentity)
...
def WellFormed (fragment : RelocatableFragment RelocKind ImportIdentity) : Prop :=
...
instance (fragment : RelocatableFragment RelocKind ImportIdentity) :
    Decidable fragment.WellFormed := by
  unfold WellFormed
  infer_instance

/-- Executable structural validation of a producer fragment. -/
def wellFormed (fragment : RelocatableFragment RelocKind ImportIdentity) : Bool :=
  decide fragment.WellFormed

@[simp] theorem wellFormed_iff
    (fragment : RelocatableFragment RelocKind ImportIdentity) :
...
structure CheckedRelocatableFragment {RelocKind : Type u}
    {ImportIdentity : Type v}
    (fragment : RelocatableFragment RelocKind ImportIdentity) : Type (max u v) where
...
structure CertifiedRelocatableFragment
    {Source : Type w} {RelocKind : Type u} {ImportIdentity : Type v}
    (Exact : Source → RelocatableFragment RelocKind ImportIdentity → Prop)
    (source : Source) (fragment : RelocatableFragment RelocKind ImportIdentity) where
