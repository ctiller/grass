...
@[simp] theorem lookup?_isSome_iff_mem_fieldNames
    (layout : StructLayout profile) (name : Name) :
    (layout.lookup? name).isSome = true ↔ name ∈ layout.fieldNames := by
  simp [lookup?, fieldNames]

/-- Every declared field name has a concrete placement lookup result. -/
theorem fieldForName (layout : StructLayout profile) (name : Name)
...
/-- Proposition-level local validity of one ordinary field placement. -/
def FieldWellFormed (layout : StructLayout profile)
    (placed : PlacedField profile) : Prop :=
  0 < placed.field.repr.size ∧
  placed.field.repr.WellFormed ∧
  IsAligned placed.offset placed.field.repr.alignment ∧
  placed.byteRange.WithinBound layout.size ∧
  layout.alignment % placed.field.repr.alignment = 0

@[simp] theorem fieldWellFormed_eq_true_iff
    (layout : StructLayout profile) (placed : PlacedField profile) :
...
/-- Proposition-level validity of the complete ordinary field family. -/
def FieldsWellFormed (layout : StructLayout profile) : Prop :=
  layout.fields ≠ [] ∧
  ∀ placed ∈ layout.fields, layout.FieldWellFormed placed

@[simp] theorem fieldsWellFormed_eq_true_iff (layout : StructLayout profile) :
    layout.fieldsWellFormed = true ↔ layout.FieldsWellFormed := by
  simp [fieldsWellFormed, FieldsWellFormed]
...
/-- Proposition-level pairwise separation of ordinary fields. -/
def FieldsDisjoint (layout : StructLayout profile) : Prop :=
  layout.fields.Pairwise fun left right =>
    left.byteRange.Disjoint right.byteRange

@[simp] theorem fieldsDisjoint_eq_true_iff (layout : StructLayout profile) :
...
theorem fieldNamesNodup_of_wellFormed (layout : StructLayout profile)
    (h : layout.WellFormed) : layout.fieldNames.Nodup :=
  (wellFormed_iff layout).mp h |>.1.1.1.1.1

theorem aggregateAlignmentPositive_of_wellFormed (layout : StructLayout profile)
    (h : layout.WellFormed) : 0 < layout.alignment :=
  (wellFormed_iff layout).mp h |>.1.1.1.1.2

theorem profileAcceptsAlignment_of_wellFormed (layout : StructLayout profile)
    (h : layout.WellFormed) :
    profile.acceptsAlignment layout.alignment = true :=
  (wellFormed_iff layout).mp h |>.1.1.1.2

theorem sizeAligned_of_wellFormed (layout : StructLayout profile)
    (h : layout.WellFormed) : IsAligned layout.size layout.alignment :=
  (wellFormed_iff layout).mp h |>.1.1.2

theorem fieldsWellFormed_of_wellFormed (layout : StructLayout profile)
    (h : layout.WellFormed) : layout.FieldsWellFormed :=
  (wellFormed_iff layout).mp h |>.1.2

theorem fieldsDisjoint_of_wellFormed (layout : StructLayout profile)
