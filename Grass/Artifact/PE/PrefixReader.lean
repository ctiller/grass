...
def readSignatureAndCoff (dosMagic dosCompatibility : Std.Logical.ByteArray)
  ...
  Grass.Artifact.Binary.continueRead (takeExact 4 input) fun signature input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 2 input) fun machine input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 2 input) fun sectionCount input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 4 input) fun timestamp input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 4 input) fun symbolTablePointer input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 4 input) fun numberOfSymbols input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 2 input) fun optionalHeaderSize input =>
  Grass.Artifact.Binary.continueRead (takeLittleEndian 2 input) fun characteristics input =>
