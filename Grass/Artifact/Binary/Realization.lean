...
/-- `takeExactSized_forget` connects sized and unsized exact-width readers,
including their suffixes and every non-success classification. -/
theorem takeExactSized_forget (count : Nat) (input : Std.Logical.ByteArray) :
    (takeExactSized count input).map Subtype.val = takeExact count input := by
  by_cases enough : count ≤ input.length
  · simp [takeExactSized, takeExact, enough, ParseResult.map]
  · simp [takeExactSized, takeExact, enough, ParseResult.map]
