...
The `trust-audit` binary in `tools/grass-tools` invokes `auditVerifiedPrograms`
from a nonce-named local command declared after importing the modules under
inspection, then requires that command's nonce marker instead of trusting the
public syntax spelling. -/
