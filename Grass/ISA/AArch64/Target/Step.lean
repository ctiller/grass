...
`.halted`. The other six families (`movz`/`movn`/`movk`, add/sub immediate
and shifted-register with `adds`/`subs`/`cmp`/`cmn` folded in via `setFlags`
and the destination register, logical immediate, `ldr`/`str`/`ldrb`/`strb`,
`cbz`) are `.internal`. `blr` through an import slot and `brk` are not
covered (`Target.Encoding` does not decode them), matching that module's
documented gap.
-/
