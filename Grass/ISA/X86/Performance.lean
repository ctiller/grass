...
what one could not, and `tools/grass-tools`'s `source-liveness` now reads the
retrieval status that the second of them depends on.
-/
structure JustifiedCostModel (Insn Vals : Type) where
  ...
  with `tools/grass-tools`'s `source-liveness` reading `livenessProbe`, the
  premise now reaches a status something other than the author can check.
  -/
...
`tools/grass-tools`'s `source-liveness` is what will notice.
-/
