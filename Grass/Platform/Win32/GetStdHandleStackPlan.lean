...
/-!
# Deterministic GetStdHandle stack-plan construction
...
def derive? {callBefore : State} {afterFetch afterRead afterStore : MachineState}
  ...
  if stackCPU : policy.stack.space = .cpuVirtual then
    ...
    if totalBound : entryRsp.toNat + WriteFile.Abi.returnAddressBytes +
        Win64.shadowSpaceBytes ≤ 2 ^ 64 then
      ...
      | .ok returnPlan =>
        let returnSlot : WriteFile.Argument :=
          ⟨policy.stack, ⟨returnPlan.offset, WriteFile.Abi.returnAddressBytes⟩⟩
        ...
        | .ok returnAccess =>
          ...
          if returnStack : returnAccess.allocation.source = .stack then
            let homeAddress := entryRsp + BitVec.ofNat 64 WriteFile.Abi.returnAddressBytes
            ...
            | .ok homePlan =>
              let homeSlot : WriteFile.Argument :=
                ⟨policy.stack, ⟨homePlan.offset, Win64.shadowSpaceBytes⟩⟩
              ...
              | .ok homeAccess =>
                if homeFits : FitsAllocation homePlan.base homeAccess.allocation.extent.stop then
                  ...
                  let returnResolved : WriteFile.Resolved call.result.machine.memory returnSlot :=
                  ...
                  let homeResolved : WriteFile.Resolved call.result.machine.memory homeSlot :=
                    ...
                    if separated : homeResolved.physical.Disjoint returnResolved.physical then
                      .ok
                        { continuation := call.fetch.site.fallthroughRip
                          ...
                          homeSize := rfl
                          homeAddress := by
                            have noWrap : entryRsp.toNat + WriteFile.Abi.returnAddressBytes <
                                2 ^ 64 := by
                              have positive : 0 < Win64.shadowSpaceBytes := by decide
                              omega
                            have numeric : homeAddress.toNat =
                                entryRsp.toNat + WriteFile.Abi.returnAddressBytes := by
                              simp only [homeAddress, BitVec.toNat_add, BitVec.toNat_ofNat]
                              rw [Nat.mod_eq_of_lt (by decide : WriteFile.Abi.returnAddressBytes < 2 ^ 64)]
...
/-- Loaded-image wrapper: the policy is the one retained by the existing
actual-CALL Windows binding rather than a separately selected input. -/
