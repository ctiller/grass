...
/-- `__NR3264_read`, `63`. -/
def sysRead : UInt64 := 63

/-- `__NR3264_write`, `64`. -/
def sysWrite : UInt64 := 64

/-- `__NR3264_brk`, `214`. -/
def sysBrk : UInt64 := 214

/-- `__NR3264_munmap`, `215`. -/
def sysMunmap : UInt64 := 215

/-- `__NR3264_mmap`, `222`. -/
def sysMmap : UInt64 := 222

/-- `__NR_clock_gettime`, `113`. -/
def sysClockGettime : UInt64 := 113

/-- `__NR_exit`, `93`. -/
def sysExit : UInt64 := 93

/-- `__NR_exit_group`, `94`. -/
def sysExitGroup : UInt64 := 94
...
read off `x0`-`x5`/`x8` instead of `rdi`/`rsi`/`rdx`/`r10`/`r8`/`r9`/`rax`. -/
def decode (call : NativeCall) : Option domain.Request :=
  ...
  | .supervisor _imm =>
    ...
    if number = sysWrite then
      ...
      some (.inl (.inl (.exit (call.reg x0).toUInt32)))
...
already modeled), so `clobbers` is empty throughout. -/
def encodeReturn (call : NativeCall) : (request : domain.Request) → domain.Response request →
    NativeReturn
  | .inl (.inl (.write _stream _bytes)), response =>
      match response with
      | .accepted count => { x0 := UInt64.ofNat count, x1 := none, writes := [], clobbers := [] }
      | .failed => { x0 := negative EIO, x1 := none, writes := [], clobbers := [] }
  | .inl (.inl (.read _stream _max)), response =>
      match response with
      | .delivered bytes =>
          { x0 := UInt64.ofNat bytes.length, x1 := none
            writes := [((call.reg x1).toNat, bytes)], clobbers := [] }
      | .failed => { x0 := negative EIO, x1 := none, writes := [], clobbers := [] }
  | .inl (.inl (.query _stream)), _response =>
      { x0 := negative ENOSYS, x1 := none, writes := [], clobbers := [] }
  | .inl (.inl (.exit _status)), response => response.elim
  | .inl (.inr (.allocate _bytes)), response =>
      match response with
      | some address => { x0 := UInt64.ofNat address, x1 := none, writes := [], clobbers := [] }
      | none => { x0 := negative ENOMEM, x1 := none, writes := [], clobbers := [] }
  | .inl (.inr (.reallocate _handle _bytes)), response =>
      match response with
      | some address => { x0 := UInt64.ofNat address, x1 := none, writes := [], clobbers := [] }
      | none => { x0 := negative ENOSYS, x1 := none, writes := [], clobbers := [] }
  | .inl (.inr (.release _handle)), response =>
      match response with
      | true => { x0 := 0, x1 := none, writes := [], clobbers := [] }
      | false => { x0 := negative EINVAL, x1 := none, writes := [], clobbers := [] }
  | .inr .now, response =>
      { x0 := 0, x1 := none
