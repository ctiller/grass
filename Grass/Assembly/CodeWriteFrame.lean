...
/-- `push_allocation` preserves every allocation record across PUSH, including the selected code record. -/
...
/-- `call_allocation` preserves every allocation record across CALL's return-address stack write. -/
