...
This module recognizes the first imported-C corpus form, `MOV dword ptr
[base+disp8], imm32`, directly from a production `DecodedSite`. Its result is
...
safety.  The opcode and addressing meanings are owned by Intel SDM Vol. 2
(`MOV`, `C7 /0`) and AMD APM Vol. 3 (`MOV`), through the existing anchors cited
by the x86 ISA modules and `docs/X86_EXECUTION.md`.
-/
...
structure Evidence (rip : BitVec 64) (bytes : ByteSeq) (state : State) where
  ...
  modrmShape : site.encoding.modrm =
    some ⟨ModRm.modDisp8, 0, base.encodingBits⟩
  noSib : site.encoding.sib = none
  displacementShape : site.encoding.disp = .d8 displacement
  immediateShape : site.encoding.imm = .i32 immediate
  baseLow : base.rexBit = false
  decodedMemory :
    decodeMem
      { mod := ModRm.modDisp8, rm := base.encodingBits, sib := none
        disp := .d8 displacement, rexX := 0, rexB := 0 } =
      some (.base base (BitVec.signExtend 32 displacement))
  atInstruction : state.rip = rip
  addressFits :
    (state.gpr base + BitVec.signExtend 64 (BitVec.signExtend 32 displacement)).toNat + 4 ≤
      2 ^ 64

namespace Evidence

def operand {rip bytes state} (e : Evidence rip bytes state) : MemOperand :=
  .base e.base (BitVec.signExtend 32 e.displacement)

theorem decodedMemory_operand {rip bytes state} (e : Evidence rip bytes state) :
    decodeMem
      { mod := ModRm.modDisp8, rm := e.base.encodingBits, sib := none
        disp := .d8 e.displacement, rexX := 0, rexB := 0 } = some e.operand :=
  e.decodedMemory
...
def address {rip bytes state} (e : Evidence rip bytes state) : BitVec 64 :=
  state.gpr e.base + BitVec.signExtend 64 (BitVec.signExtend 32 e.displacement)
