...
/-- Expose both component derivations of a sequential format without losing
the result-type index. -/
theorem seqOuterShape {α : Type} {format : Format α}
    {input : Std.Logical.ByteArray} {value : α} {rest : Std.Logical.ByteArray}
    (derivation : Derives format input value rest) :
    match format with
    | .seq first next =>
        ∃ middle leftValue rightValue,
          ...
          Derives first input leftValue middle ∧
          Derives (next leftValue) middle rightValue rest
    | _ => True := by
  induction derivation with
  | pure => trivial
  | byte => trivial
  | seq left right => exact ⟨_, _, _, rfl, left, right⟩
  | choiceLeft => trivial
  | choiceRight => trivial
  | repeatZero => trivial
  | repeatSucc => trivial
