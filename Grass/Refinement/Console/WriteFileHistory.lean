...
def HandoffRelation (projection : CapturedTargetProjection spec Status) :=
  Realization → ProtocolState → (call : CallProtocol.CallId) →
    (record : CallProtocol.Pending Request) → projection.componentSystem.History →
    (state : ProtocolState) → Prefix plan state call record → Prop

/-- Follow the original root, not an independently reconstructed handoff. -/
def RootAligned (relation : HandoffRelation (plan := plan) projection)
    (upper : projection.componentSystem.History) {state : ProtocolState}
    {frontier : Prefix plan state call record} : History plan realization initial call record frontier → Prop
...
private def positiveUpper {before after : ProtocolState}
...
private def advanceUpper {before after : ProtocolState}
    {pre : Prefix plan before call record} {post : Prefix plan after call record}
    (start : Start projection record) (action : Action) (output : Vec Byte)
    (step : CommittedStep realization pre post action output)
    (upper : Located projection (WriteFileProjection.cut start.cut start.suffix pre)) :
...
private def fold (start : Start projection record) {state : ProtocolState}
...
structure Aligned (relation : HandoffRelation (plan := plan) projection)
...
variable {relation : HandoffRelation (plan := plan) projection}
  {state : ProtocolState} {frontier : Prefix plan state call record}
  {history : History plan realization initial call record frontier}
...
def extend (aligned : Aligned relation history) {after : ProtocolState}
    {post : Prefix plan after call record} (action : Action) (output : Vec Byte)
...
private theorem history_transport
    (other : Σ otherState, Prefix plan otherState call record)
    (pointEq : other = ⟨state, frontier⟩)
    (otherHistory : History plan realization initial call record other.2)
...
theorem extend_start (aligned : Aligned relation history) {after : ProtocolState}
    {post : Prefix plan after call record} (action : Action) (output : Vec Byte)
    (step : CommittedStep realization frontier post action output) :
...
theorem extend_zero (aligned : Aligned relation history) {after : ProtocolState}
    {post : Prefix plan after call record} (action : Action) (output : Vec Byte)
    (step : CommittedStep realization frontier post action output)
...
def publicationPath (aligned : Aligned relation history) {after : ProtocolState}
    {post : Prefix plan after call record} (action : Action) (output : Vec Byte)
    (step : CommittedStep realization frontier post action output)
...
theorem extend_positive (aligned : Aligned relation history) {after : ProtocolState}
    {post : Prefix plan after call record} (action : Action) (output : Vec Byte)
    (step : CommittedStep realization frontier post action output)
    (positive : frontier.accepted < post.accepted) :
...
theorem extend_positive_events (aligned : Aligned relation history)
    {after : ProtocolState} {post : Prefix plan after call record}
