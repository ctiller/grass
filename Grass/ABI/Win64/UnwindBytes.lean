...
own toolchain. The `win64-unwind-differential` binary in `tools/grass-tools`
assembles MASM `PROC FRAME` prologues with `ml64.exe`, reads the `.xdata`
section straight out of the COFF object, and compares it byte for byte against
`UnwindInfo.toBytes`.
